...
pcall(function()
	if Tabs and Tabs.Main then
		...
		-- Dropdown chọn vị trí spawn (1 → 9)
		FarmTab:AddDropdown("SpawnSelect", {
			...
			Callback = function(value)
				...
				-- ✅ Nếu đang bật Auto Spawn thì tự cập nhật luôn
				if AutoSpawnConfig.Enabled then
					task.defer(autoSpawn)
				end
			end
...
States.Fruit.AutoDrop = States.Fruit.AutoDrop or false
States.Fruit.AutoBring = States.Fruit.AutoBring or false
States.Drink = States.Drink or {}
States.Drink.AutoDrink = States.Drink.AutoDrink or false

local autoBarrelTPThread
local autoDrinkThread

--// Blacklist vị trí thùng
local blacklist = {
...
--// Hàm bring thùng thay vì TP
local function StartAutoBarrelBringLoop(barrels)
            ...
            if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                ...
                for _, b in ipairs(barrels:GetDescendants()) do
                    ...
                    if b:IsA("BasePart") and not isBlacklisted(b.Position) then
                        if (b.Position - hrp.Position).Magnitude <= States.Fruit.BarrelRange then
                            local click = b:FindFirstChildOfClass("ClickDetector")
                            -- Bring thùng lại gần người chơi
                            pcall(function()
                                b.CFrame = hrp.CFrame * CFrame.new(0, 0, -3)
                            end)
                            task.wait(0.05)
                            if click then
